...
private:
	bool checkname(const char(&buf1)[300]) {
		int n; bool er1 = true;
		n = strlen(buf1);
		if ((n > 30) || (n == 0)) { cout << "Ошибка: нарушение допустимой длины строки.\n"; return false; }
		if ((((int)buf1[0] < -64) || ((int)buf1[0] > -33)) && ((int)buf1[0] != -88)) { cout << "Ошибка: неправильный 1-ый символ.\n"; return false; }
		for (i = 1; i <= (n - 1); i++) {
		...
		if (er1 == false) { cout << "Ошибка: ввод недопустимых символов.\n"; return false; }
		return true;
	}
	bool checknomer(const char(&buf2)[300]) {
		...
		if (buf2[0] != '8')
		{
			if ((buf2[0] != '+') || (buf2[1] != '7')) { cout << "Ошибка: неправильное начало номера.\n"; return false; }
		}
		if (((buf2[0] == '8') && (n1 != 11)) || ((buf2[0] == '+') && (buf2[1] == '7') && (n1 != 12)))
		{ cout << "Ошибка: недопустимая длина номера.\n"; return false; }
		for (i = 1; i <= (n1 - 1); i++) {
			if (((int)buf2[i] < 48) || ((int)buf2[i] > 57)) {
				er2 = false; break;
			}
		}
		if (er2 == false) { cout << "Ошибка: ввод недопустимых символов.\n"; return false; }
		return true;
	}
public:
	void operation(char x) {
		if (x == '1') {
			...
			do { cout << "Введите отчество: ";
				cin.getline(name3, 300);
				OemToAnsi(name3, name3); N = checkname(name3);
			} while (N == false); 
			do { cout << "Введите номер телефона: "; cin.getline(nomer, 300); N = checknomer(nomer); } while (N == false);
...
private:
	...
	bool cheksearch(const char(&buf3)[300]) {
		...
		if (n2 > 92) { cout << "Ошибка: задан слишком длинный запрос.\n"; return false; }
		if (n2 == 0) { cout << "Ошибка: задан пустой запрос.\n"; return false; }
		...
		if (er3 == false) { cout << "Ошибка: ввод недопустимых символов.\n"; return false; }
		if (j == n2) { cout << "Ошибка: задан пустой запрос.\n"; return false; }
		return true;
		 }
public:
	void operation(char x) {
		if (x == '2') {
			...
			bool search; 
			do { cout << "Введите поисковый запрос: ";
			...
			fin.close();
			if (foun == false) cout << "По Вашему запросу не нашёлся ни один контакт.\n";
		}
	}
};
int main() {
	setlocale(LC_ALL, "Russian");
	cout << "Для записи нового контакта в справочник введите 1.\nДля нахождения контакта в справочнике введите 2.\nДля окончания работы со справочником введите 3.\n";
